...
export class GenerateEncryptedTokenService {
  ...
  async exec<T = Record<string, any>>(params: TGenerateEncryptedTokenServiceParams<T>): Promise<string> {
    ...
    const token = await this._tokenService.generateToken(encryptedPayload, params.exp);
